...
사용 데이터:
    StudentsPerformance.csv  (제안서에서 사용한 Kaggle 데이터셋과 동일)
"""
...
def load_data(path: str = DATA_PATH) -> pd.DataFrame:
    """CSV를 읽고 기본적인 정리 및 파생변수(average_score)를 만든다."""
    df = pd.read_csv(path)
...
def encode_features(df: pd.DataFrame):
    ...
    # 학업 성취도와 직접 관련된 성적 컬럼은 설명변수에서 제외
...
def descriptive_stats(df: pd.DataFrame):
    """기술 통계와 그룹별 평균, 박스플롯 등을 생성한다."""
    OUTPUT_DIR.mkdir(exist_ok=True)

    # 전체 점수 요약 통계
    summary = df[["math_score", "reading_score", "writing_score", "average_score"]].describe()
    ...
    with open(OUTPUT_DIR / "group_means.txt", "w", encoding="utf-8") as f:
        for col in group_cols:
            ...
            means = df.groupby(col)["average_score"].mean().sort_values(ascending=False)
            f.write(str(means))
            f.write("\n")
...
def run_random_forest(X, y, feature_cols):
    ...
    importances.to_csv(OUTPUT_DIR / "rf_feature_importances.csv", encoding="utf-8-sig")
...
def run_anova(df: pd.DataFrame):
    """
    성별, 인종, 부모 학력, 점심, 시험 준비가 average_score에 미치는
    영향을 ANOVA로 검정.
    """
    formula = (
    ...
    anova_table.to_csv(OUTPUT_DIR / "anova_table.csv", encoding="utf-8-sig")
